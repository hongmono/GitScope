...
- 브랜치 사이드바, 커밋 목록, 커밋 상세로 이루어진 macOS 표준 3분할 화면
- 밝은 모드와 어두운 모드 모두 지원
- `⌘B`로 브랜치 사이드바, `⇧⌘B`로 커밋 상세 여닫기
- `⌘,` 설정에서 자동 가져오기 주기, 작성자 아바타 원격 조회, GitHub Actions 상태 조회 개별 조정
...
- 브랜치가 다른 lane으로 옮겨가도 같은 색을 유지하는 그래프 색상
...
- ⌘클릭/⇧클릭 커밋 다중 선택으로 변경 파일 합집합 보기, 파일별 커밋 patch 이어보기
- 서로 다른 저장소의 커밋을 함께 선택하면 변경 파일을 저장소별로 구분 표시
- 커밋 메시지, 작성자, 시간 표시
- GitHub 저장소 커밋의 Actions 상태를 커밋 메시지 오른쪽에 표시
- Actions 실행 중 자동 갱신, 워크플로 및 Job/Check 결과와 GitHub 링크 제공
- 메시지(제목·본문)/작성자/해시 검색, 작성자/기간/저장소 필터
- 저장소당 최근 2,000개 커밋 로드, 상한에 도달하면 목록 하단에 안내 표시
